...
def preprocess_raw_image(x):
    ...
    # x = torch.nn.functional.interpolate(x, 518 * (resolution // 256), mode='bicubic')
    return x


def load_dinov2_encoder(device="cuda", resolution=512):
    from dinov2.models.vision_transformer import vit_small
    dinov2_vitb14 = vit_small(patch_size=14, img_size=518) 
    state_dict_vitb14 = torch.load("/data2/yifan/Jodi-main/dinov2_vits14_pretrain.pth")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                with torch.amp.autocast(
                    ...
                    z = []
                    # [4, 3, 3, 64, 64] 2 imgs[0] [4, 3, 64, 64]
...
#################################################################################
                imgs = batch["img"]          # [B, K+1, C, H, W]
                roles = batch["role"]        # [B, K+1]
                
                B, K_plus_1, C, H, W = imgs.shape
                imgs_flat = imgs.view(B * K_plus_1, C, H, W)        # [B*(K+1), C, H, W]
                roles_flat = roles.view(B * K_plus_1)               # [B*(K+1)]
                
                role0_mask = (roles_flat == 0)
                role0_imgs = imgs_flat[role0_mask]                  # [N0, C, H, W], N0<=B*(K+1)
                raw_image_ = preprocess_raw_image(role0_imgs)
                raw_image_ = raw_image_.to(accelerator.device) 
                encoder = load_dinov2_encoder(device='cuda', resolution=512)
...
#####################################################################################                
                ...
                loss_term = train_diffusion.training_losses(
                    model, clean_images, timesteps, z,
            ...
            if (step + 1) % config.train.log_interval == 0 or (step + 1) == 1:
                ...
                epoch_yes = global_step // train_dataloader_len + 1
                
                info = (
                    f"Epoch: {epoch_yes} | Global Step: {global_step} | Local Step: {current_step} // {train_dataloader_len}, "
...
def main(cfg: JodiConfig) -> None:
    ...
    if state_dict is not None:
        ...
        if args.resume_from_sana:
            ...
            # 加载 final_layer 的线性层 + 额外的 scale_shift_table
            # final_layers[0] 是 第一个用于无条件或基础条件的输出头
        ...
        # 扩展到新领域（加载部分参数 + 复制旧领域的权重）

            # 得到已有的“老领域”数量（即旧模型里有几个 domain）我知道了 这是用来再加模态去训练的
