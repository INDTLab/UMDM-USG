    ...
    for epoch in range(start_epoch + 1, config.train.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if torch.isnan(loss):
                    logger.warning("Detected NaN loss!")
                    logger.warning(f"Epoch: {epoch}, Global Step: {global_step}, Step: {step}")
                    logger.warning(f"role mean: {role.float().mean().item():.4f}, std: {role.float().std().item():.4f}")
                    logger.warning(f"y_mask mean: {y_mask.float().mean().item():.4f}, shape: {y_mask.shape}")
                    logger.warning(f"timesteps: {timesteps.cpu().tolist()}")
                    logger.warning(f"text[0]: {batch['text'][0]}")
                    logger.warning(f"clean_images norm: {clean_images.norm():.4f}")
                    
                    for k, v in loss_term.items():
                          try:
                              logger.warning(f"loss_term[{k}]: mean={v.mean().item():.6f}, std={v.std().item():.6f}, max={v.max().item():.6f}, min={v.min().item():.6f}")
                          except Exception as e:
                              logger.warning(f"loss_term[{k}] error: {e}")
