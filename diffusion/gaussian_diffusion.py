...
def compute_projection_loss(z, z_tilde, normalize=True):
    """
    Compute projection loss between z and z_tilde.
    Inputs:
        z         : Tensor of shape (B, T, Z), projected feature (target)
        z_tilde   : Tensor of shape (B, T, Z), projected prediction
        normalize : Whether to apply L2 normalization on the last dim
    Output:
        proj_loss : Scalar tensor (averaged over B and T)
    """
    # assert z.shape == z_tilde.shape, "Shape mismatch between z and z_tilde"
    if z.shape != z_tilde.shape:
        B, T_tilde, Z = z_tilde.shape
        z = z[:, :T_tilde, :]
    ...
    # Cosine similarity: (z * z_tilde).sum over dim Z => shape (B, T)
    similarity = (z * z_tilde).sum(dim=-1)

    # Projection loss = - mean cosine similarity
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, timestep, z, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        # 获取当前步的噪声图像 如果不是 skip_noise（常规流程），就通过扩散过程 q(x_t|x_0) 生成噪声图像
        if skip_noise:
            x_t = x_start
        # 如果传入了 noise 就用它，否则自己采样
        else:
            if noise is None:
                noise = th.randn_like(x_start)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            proj_loss = compute_projection_loss(z, zs)
            proj_loss = proj_loss.mean() 
            terms["proj"] = proj_loss
            ...
            if "vb" in terms:
                ...
                # combined_loss = 0.8 * mse_loss + 0.2 * proj_loss
                combined_loss = 0.8 * terms["mse"] + 0.2 * terms["proj"]
                terms["combined_loss"] = combined_loss
                terms["loss"] = terms["combined_loss"]
