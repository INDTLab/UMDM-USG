...
class Block(nn.Module):
    ...
    def forward(
        self, hidden_states: Tensor, residual: Optional[Tensor] = None, mask: Optional[torch.BoolTensor] = None, inference_params=None
):
        ...
        # mask = True to residual mask=False to mixer
        
            mask_expanded = mask.unsqueeze(-1).expand_as(hidden_states)
            hidden_states = torch.where(mask_expanded, residual, hidden_states)
...
def v_block(
    ...
    # import ipdb; ipdb.set_trace()
    mixer_cls = partial(Mamba, d_state=d_state, bimamba_type=bimamba_type, if_divide_out=if_divide_out, init_layer_scale=init_layer_scale, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
    ...
    block.apply(partial(_init_weights, n_layer=1))
...
def _init_weights(
    ...
    if rescale_prenorm_residual:
        ...
        #   > the weights of residual layers at initialization by a factor of 1/¡ÌN where N is the # of residual layers.
