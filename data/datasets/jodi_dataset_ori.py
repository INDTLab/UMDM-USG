...
class JodiDataset(Dataset):
    ...
    def load_data(self):
        ...
        data = list(data.items())   # [(image_id_1, info_1), (image_id_2, info_2), ...]
        return data
    ...
    def __getitem__(self, index: int):
        ...
        # role：标签0(目标) 1(条件) 2(空)   cnt: 记录当前找到了多少个有效条件图
        ...
        for condition in self.conditions:
            ...
            # 如果该条件图找不到
            if ann_path is None:
                # 如果是人体估计图找不到了 那就创建纯黑图，不算空，算条件图 role后面决定
                if self.use_empty_openpose_image and "openpose" in condition:
                    ...
                    role.append(None)
                    cnt += 1
                # 如果是其他图找不到了，比如说对应的mask depth什么的，就用全零表示空 直接role=2
                else:
                    img.append(torch.zeros_like(img[0]) if isinstance(img[0], torch.Tensor) else None) # 全0
                    role.append(2)

            # 如果该条件图存在 则有效不做处理，后面再分配role
            else:
